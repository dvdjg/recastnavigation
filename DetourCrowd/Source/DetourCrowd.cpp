						 ...
						 dtCrowdAgent** agents, const int /*nagents*/, dtProximityGrid* grid)
{
	...
	for (int i = 0; i < nids; ++i)
	{
		...
		if (dtMathFabs(diff[1]) >= (height+ag->params.height)/2.0)
			continue;
		diff[1] = 0;
...
void dtCrowd::updateReinsertToNavmesh(const double dt)
{
...
void dtCrowd::updateHandleCollisions()
{
...
void dtCrowd::updateComputeDesiredPosition(const double dt, dtCrowdAgentDebugInfo* debug)
{
    m_velocitySampleCount = 0;

    dtCrowdAgent** agents = m_activeAgents;
    const int nagents = m_currentAgentCount;
    m_currentAgentCount = getActiveAgents(agents, m_maxAgents);
