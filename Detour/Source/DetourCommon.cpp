...
bool dtClosestHeightPointTriangle(const double* p, const double* a, const double* b, const double* c, double& h)
{
	...
	const double dot00 = dtVdot2D(v0, v0);
	const double dot01 = dtVdot2D(v0, v1);
	const double dot02 = dtVdot2D(v0, v2);
	const double dot11 = dtVdot2D(v1, v1);
	const double dot12 = dtVdot2D(v1, v2);
						 ...
						 double& s, double& t)
{
    ...
    if (dtMathFabs(d) < 1e-6f) return false;
	s = vperpXZ(v,w) / d;
	t = vperpXZ(u,w) / d;
	return true;
}
