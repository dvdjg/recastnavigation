...
dtStatus dtNavMeshQuery::findRandomPointAroundCircle(dtPolyRef startRef, const double* centerPos, const double radius,
...
dtStatus dtNavMeshQuery::closestPointOnPoly(dtPolyRef ref, const double* pos, double* closest, bool* posOverPoly) const
{
	...
	if (poly->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)
	{
		...
		const double d0 = dtVdist(pos, v0);
		const double d1 = dtVdist(pos, v1);
...
dtStatus dtNavMeshQuery::getPolyHeight(dtPolyRef ref, const double* pos, double* height) const
{
	...
	if (poly->getType() == DT_POLYTYPE_OFFMESH_CONNECTION)
	{
		const double* v0 = &tile->verts[poly->verts[0]*3];
		const double* v1 = &tile->verts[poly->verts[1]*3];
		const double d0 = dtVdist2D(pos, v0);
		const double d1 = dtVdist2D(pos, v1);
		const double u = d0 / (d0+d1);
		if (height)
...
///
dtStatus dtNavMeshQuery::findNearestPoly(const double* center, const double* extents,
...
int dtNavMeshQuery::queryPolygonsInTile(const dtMeshTile* tile, const double* qmin, const double* qmax,
										const dtQueryFilter* filter,
