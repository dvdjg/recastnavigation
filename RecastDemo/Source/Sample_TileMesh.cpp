...
void Sample_TileMesh::handleRenderOverlay(double* proj, double* model, int* view)
{
	...
	if (m_tileBuildTime > 0.0 && gluProject((GLdouble)(m_tileBmin[0]+m_tileBmax[0])/2, (GLdouble)(m_tileBmin[1]+m_tileBmax[1])/2, (GLdouble)(m_tileBmin[2]+m_tileBmax[2])/2,
											 model, proj, view, &x, &y, &z))
	{
		char text[32];
...
unsigned char* Sample_TileMesh::buildTileMesh(const int tx, const int ty, const double* bmin, const double* bmax, int& dataSize)
{
	...
	m_ctx->log(RC_LOG_PROGRESS, " - %.1fK verts, %.1fK tris", nverts/1000.0, ntris/1000.0);
